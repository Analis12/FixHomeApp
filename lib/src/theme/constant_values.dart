import 'package:flutter/material.dart';

//Style
const headerStyle = TextStyle(fontSize: 22, fontWeight: FontWeight.bold);
const headerStyle2 = TextStyle(fontSize: 45, fontWeight: FontWeight.w500);
const formStyle = TextStyle(fontSize: 25, fontWeight: FontWeight.w500);
const cardStyle = TextStyle(fontSize: 22, fontWeight: FontWeight.w500);
const cardtitleStyle = TextStyle(fontSize: 25, fontWeight: FontWeight.bold);
const titleStyle = TextStyle(fontSize: 17, fontWeight: FontWeight.w400);
const titleStyle2 = TextStyle(fontSize: 16, color: Colors.black45);
const subtitleStyle = TextStyle(fontSize: 14, fontWeight: FontWeight.w500);
const infoStyle = TextStyle(fontSize: 12, color: Colors.black54);
const StyleSettings = TextStyle(fontSize: 22, fontWeight: FontWeight.w500);
//Decoration
final roundedRectangle12 = RoundedRectangleBorder(
  borderRadius: BorderRadiusDirectional.circular(12),
);

final roundedRectangle4 = RoundedRectangleBorder(
  borderRadius: BorderRadiusDirectional.circular(4),
);
